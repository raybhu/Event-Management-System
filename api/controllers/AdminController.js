...
module.exports = {
  ...
  update: async function (req, res) {
    ...
    if (typeof ExistedEvent === 'undefined') {
      return res.badRequest('The Event is not existed.');
    }
    ...
    if (req.method === 'GET') {
      ...
      if (typeof req.body.Event === 'undefined') {
        ...
        log = {
          title: 'Error!',
          content: 'Sorry, you failure updated an event. Failure reasons: The Event model is undefined!'
        };
      } else if (typeof req.body.Event.organizer === 'undefined') {
        isSuccessfulUpdatedEvent = false;
        log = {
          title: 'Error!',
          content: 'Sorry, you failure updated an event. Failure reasons: The Event Organizer must be selected!'
        };
      } else if (typeof req.body.Event.venue === 'undefined') {
        isSuccessfulUpdatedEvent = false;
        log = {
          title: 'Error!',
          content: 'Sorry, you failure updated an event. Failure reasons: The Venue must be selected!'
        };
      } else {
        ...
        log = {
          title: 'Well done!',
          content: 'Aww yeah, you successfully updated an event!'
        };
      }
      var viewPath = '';
  ...
  delete: async function (req, res) {
    if (req.method === 'GET') {
      ...
      return res.view('pages/Admin', {
        ...
        log: {
          title: 'Well done!',
          content: 'Aww yeah, you successfully Deleted an event!'
        },
        isUpdate: false,
        events: eventModels,
        layout: 'layouts/bootstrap',
  ...
  search: async function (req, res) {
    ...
    var userModels = await Event.find({
      id: event.id
    }).populate('beRegister');
    res.status(200);
    return res.json({
      registers: userModels[0].beRegister
    });
  },
};
